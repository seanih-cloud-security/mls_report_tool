...
# * MAIN LOGIC
# 1. Load data
df = pd.read_csv("data/mock_mls_report_4_months.csv")
# df_xlxs = pd.read_excel("data/mock_mls_report.xlsx")
...
# print(f"avg sale price: ${avg_sale_price:,}")
# print(f"median sale price: ${median_sale_price:,}")
# print(f"sale price sum: ${closing_price_sum:,}")
# print(f"avg days on market: {avg_days_on_market:.1f}")
...
absorption_rate = closed_sales_count / timeframe_months if timeframe_months > 0 else 0

# Calculate Months of Supply
if absorption_rate > 0:
    months_of_supply = active_inventory / absorption_rate
else:
    months_of_supply = float("inf")  # Avoid division by zero if 0 sales occurred

# print("---")
# print(f"Active Inventory: {active_inventory} listings")
# print(f"Closed Sales: {closed_sales_count} listings")
# print(f"Absorption Rate: {absorption_rate:.2f} sales/month")
# print(f"Months of Supply: {months_of_supply:.2f} months")

# * AVG $/SQ FT & SALE-TO-LIST RATIOS
# 1. Convert columns to numeric, coercing bad data to NaN
closed_df["ClosePrice"] = pd.to_numeric(closed_df["ClosePrice"], errors="coerce")

closed_df["LivingArea"] = pd.to_numeric(closed_df["LivingArea"], errors="coerce")
...
valid_sqft_mask = (closed_df["LivingArea"] > 0) & (closed_df["ClosePrice"].notnull())
...
# print("---")
# print(f"Average Sale-to-List Ratio: {avg_sale_to_list:.2f}%")
# print(f"Median Sale-to-List Ratio:  {median_sale_to_list:.2f}%")
# print(f"Average Sale-to-Original-List Ratio: {avg_sale_to_orig:.2f}%")
# print(f"Median Sale-to-Original-List Ratio: {median_sale_to_orig:.2f}%")
# print(f"Average $/sq ft: ${avg_price_per_sqft:.2f}")
...
if total_closed_sales > 0:
    # 4. Categorize sales
    above_asking_count = (valid_closed["ClosePrice"] > valid_closed["ListPrice"]).sum()
    at_asking_count = (valid_closed["ClosePrice"] == valid_closed["ListPrice"]).sum()
    below_asking_count = (valid_closed["ClosePrice"] < valid_closed["ListPrice"]).sum()
    ...
    # print("---")
    # print(f"Total Closed Analyzed: {total_closed_sales}")
    # print(f"Above Asking Price: {pct_above:.2f}% ({above_asking_count} homes)")
    # print(f"At Asking Price:    {pct_at:.2f}% ({at_asking_count} homes)")
    # print(f"Below Asking Price: {pct_below:.2f}% ({below_asking_count} homes)")
else:
...
formatted_trends["Avg_DOM"] = formatted_trends["Avg_DOM"].apply(lambda x: f"{x:,.1f}")
...
with open("mls_report.txt", "w") as file:
    file.write(final_report_string)


# * GUI SECTION
# window = tk.Tk()
# window.title("Excel Processing App")
# window.geometry("600x250")

# selected_file = tk.StringVar()

# title_label = tk.Label(
#     window, text="Select an Excel file to process:", font=("Arial", 14)
# )
